<template>
  <v-card class="mx-auto" >
    ...
    <v-window v-model="step" >
      <v-window-item :value="i+1" 
      v-for="(obj, i ) in dataFrm.others"
      :key="`item${i}`">
      ...
      <v-window-item :value=" i + dataFrm.others.length + 1"
      v-for="(obj, i ) in dataFrm.sections"
      :key="`sections${i}`"
      >
      ...
      <div v-for="(o, j) in obj.questions" :key="`enu${i}_${j}`">
        ...
        <v-card-text>
          <h4>{{ i + 1 + dataFrm.others.length }}.{{ j + 1 }}.- {{ o.question }}</h4>
